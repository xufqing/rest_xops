...
class Tailf(object):
    ...
    def remote_tail(self, host, port, user, passwd, logfile, webuser):
        # 创建一个可跨文件的全局变量，控制停止
        gl._init()
        ...
        try:
            ...
            interact.expect('.*#.*')
            logfile = logfile.strip().replace('&&', '').replace('||', '')
            interact.send('tail -f %s' % (logfile))
            interact.tail(output_callback=lambda m: self.send_message(webuser, m), stop_callback=lambda x: gl.get_value('tail_' + str(webuser)) )
        except Exception as e:
            self.send_message(webuser, e)
        finally:
            try:
